...
async def get_wireguard_status(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Error obteniendo estado WireGuard"
        )
...
"""
        ...
        return {
            "success": True,
            "message": "WireGuard iniciado exitosamente"
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error iniciando WireGuard"
        )
...
async def stop_wireguard(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        return {
            "success": True,
            "message": "WireGuard detenido exitosamente"
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error deteniendo WireGuard"
        )
    except Exception as e:
...
async def get_wireguard_peers(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error obteniendo peers WireGuard"
        ) 