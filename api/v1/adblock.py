...
async def get_adblock_status(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Error obteniendo estado AdBlock"
        )
...
async def enable_adblock(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        return {
            "success": True,
            "message": "AdBlock habilitado exitosamente"
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error habilitando AdBlock"
        )
...
async def disable_adblock(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        return {
            "success": True,
            "message": "AdBlock deshabilitado exitosamente"
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error deshabilitando AdBlock"
        )
    ...
    try:
        ...
        if os.path.exists(script_path):
            ...
            if result.returncode == 0:
                ...
                return {
                    "success": True,
                    "message": "Listas AdBlock actualizadas exitosamente"
                }
            else:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Error actualizando listas: {result.stderr}"
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Script de actualización no encontrado"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Timeout actualizando listas AdBlock"
        )
    except Exception as e:
        logger.error(f"Error actualizando listas AdBlock: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error actualizando listas AdBlock"
        )
...
async def get_adblock_config(current_user: Dict[str, Any] = Depends(get_current_active_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error obteniendo configuración AdBlock"
        )
...
async def update_adblock_config(
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Configuración AdBlock actualizada exitosamente"
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error actualizando configuración AdBlock"
        )
